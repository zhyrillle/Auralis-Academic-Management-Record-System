...
const ROLE_CONFIG = {
  "dept-head": {
    pageTitle: "Department Head Feedback",
    badgeRole: (person) => person.dept || "Department Head",
    likertQuestions: [
    ...
    openEnded: [
      "What are the department head's strongest qualities or contributions?",
      "What aspects can the department head improve on professionally?",
    ],
  },
  "principal": {
    pageTitle: "Principal Feedback",
    badgeRole: () => "Principal",
    likertQuestions: [
      "The principal communicates clear goals and vision for the school.",
      "The principal provides constructive feedback to help teachers improve.",
      "The principal supports teachers in resolving classroom and administrative issues.",
      "The principal creates an environment where teachers feel respected and valued.",
      "The principal promotes professional development opportunities for teachers.",
      "The principal is approachable and open to teachers' concerns and suggestions.",
      "The principal makes fair and transparent decisions.",
      "The principal ensures that school policies are implemented consistently.",
    ],
    openEnded: [
      "What does the principal do well that positively impacts teaching and learning?",
      "What areas can the principal improve on to better support teachers and students?",
    ],
  },
};
...
export default function RoleEvalForm({ formType, person, onBack, onCancel }) {
  const config = ROLE_CONFIG[formType] || ROLE_CONFIG["dept-head"];
  const { pageTitle, badgeRole, likertQuestions, openEnded } = config;

  const [ratings, setRatings] = useState(Array(likertQuestions.length).fill(null));
  ...
  const validate = () => {
    const errs = {};
    ratings.forEach((r, i) => { if (r === null) errs[`q${i}`] = true; });
    answers.forEach((a, i) => { if (!a.trim()) errs[`oe${i}`] = true; });
    return errs;
  };
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(errs).length > 0) {
      setErrors(errs);
      const first = document.querySelector(".tef-error");
    ...
    <div className="tef-container">
      ...
      <div className="tef-header">
        <div className="tef-header-left">
          ...
          <h1 className="tef-page-title">{pageTitle}</h1>
        </div>
      ...
      <div className="tef-form-card">
          ...
          {person && (
            <div className="tef-teacher-badge">
              ...
              <div>
                ...
                <p className="tef-teacher-meta">{badgeRole(person)}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="tef-part">
          ...
          <div className="tef-likert-list">
            {likertQuestions.map((question, i) => (
              <div
                key={i}
                className={`tef-likert-row${errors[`q${i}`] ? " tef-error" : ""}`}
              >
                <p className="tef-q-text">{i + 1}. {question}</p>
        ...
        <div className="tef-part">
          ...
          {openEnded.map((label, i) => (
            <div
              key={i}
              className={`tef-open-group${errors[`oe${i}`] ? " tef-error" : ""}`}
            >
              <label className="tef-open-label">{label}</label>
              <textarea
