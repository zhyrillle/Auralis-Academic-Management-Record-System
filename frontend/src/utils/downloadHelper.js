 ...
 * 
 * @param {Object} selectedClass - The currently active class metadata.
...
export function downloadGradingSheetCSV(
  ...
  csvContent += `Grade & Section: ${selectedClass.gradeLevel} - ${selectedClass.sectionName},School Year: 2026-2027\r\n`;
  csvContent += `Teacher: Harvey Babia,Subject: ${selectedClass.subject}\r\n\r\n`;
  ...
  students
    ...
    .forEach((s) => {
      ...
      csvContent += `"${s.lastName}, ${s.firstName} ${
  ...
  link.setAttribute(
    "download",
    `Grading_Sheet_${selectedClass.gradeLevel}_${selectedClass.sectionName}.csv`
  );
